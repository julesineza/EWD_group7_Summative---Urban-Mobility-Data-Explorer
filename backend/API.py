...
def summary():
    ...
    join_sql = "JOIN zones z ON trips.pickup_zone_id = z.zone_id" if borough else ""
    where_sql = "WHERE z.borough = ?" if borough else ""
    args = (borough,) if borough else ()
    data = query(f"""
        SELECT
...
def top_pickup_zones():
    ...
    where_sql = "WHERE z.borough = ?" if borough else ""
    args = (borough, limit) if borough else (limit,)
    rows = query(f"""
        SELECT
            z.zone_name,
            z.borough,
...
def fare_vs_distance():
    ...
    borough = request.args.get('borough')
    join_sql = "JOIN zones z ON trips.pickup_zone_id = z.zone_id" if borough else ""
    extra_and = "AND z.borough = ?" if borough else ""
    args = (borough,) if borough else ()
    rows = query(f"""
        SELECT
            trip_distance,
...
def top_routes():
    ...
    where_sql = "WHERE pz.borough = ?" if borough else ""
    args = (borough, limit) if borough else (limit,)
    rows = query(f"""
        SELECT
...
def geojson():
    ...
    if borough:
        data['features'] = [f for f in data['features']
                            if f.get('properties', {}).get('borough') == borough]
